...
<head>
<meta charset="UTF-8">
<title>Desafio 7: Cadastro inteligente</title>
...
<p style="color: black">
Um sistema de atendimento virtual está sendo atualizado para lidar melhor com entradas de texto fornecidas pelos usuários. Atualmente, muitos clientes digitam seus dados de formas inconsistentes: alguns colocam o nome todo em maiúsculas, outros colocam espaços a mais, alguns digitam com erros de digitação e outros fornecem apenas parte das informações. Seu trabalho é criar um módulo inicial de processamento textual responsável por padronizar e analisar esses dados.<br><br>

<b>O programa deve solicitar três informações do usuário:</b><br>
<b>1.</b> Nome completo (com espaços).<br>
<b>2.</b> Cidade onde mora.<br>
<b>3.</b> Uma palavra-chave de identificação (sem espaços).<br>
Essas informações podem vir escritas de qualquer forma: tudo maiúsculo, tudo minúsculo, com espaços extras no início ou no final etc.<br><br>

<b>Checklist de Implementação</b><br>
<b>1.</b> Remover espaços extras no início e no final do nome e da cidade.<br>
<b>2.</b> Verificar se a palavra-chave coincide com a primeira palavra do nome.<br>
<b>3.</b> Juntar as informações em uma frase padronizada usando strcat e strcpy.<br>
<b>4.</b> Mostrar também quantos caracteres o nome possui (sem contar o '\0').<br><br>

<b>Objetivo</b><br>
Ao final, o programa deve ser capaz de ler textos com e sem espaços, manipulá-los caractere a caractere, comparar palavras, concatenar fragmentos e medir comprimentos, demonstrando domínio real sobre operações fundamentais com strings em C.
</p>
...
<a href="modulo7.php" class="btn-nav">⟵ Voltar</a>
      </div>
...
fetch('../../backend/update_time.php', {
  ...
  body: JSON.stringify({ modulo_id: 7 })
});
...
### OBJETIVO
Avaliar o desafio "Análise de Linha de Montagem". Nota: 0 a 100.

### REQUISITOS TÉCNICOS:
1. Uso obrigatório do laço 'for' para processar as 50 unidades.
2. Uso de um **Acumulador** (soma dos tempos) para calcular a média.
3. Uso de um **Contador** para unidades com tempo > 12 minutos.
4. Uso de uma **Flag** (sinalizador/booleano) para detectar se houve algum caso crítico (> 20 min).
5. Cálculo da média final (Soma total / 50).

### TABELA DE PONTUAÇÃO (TOTAL: 100 PTS)
1. **Lógica de Repetição (for) (25 pts):** Implementação correta do laço para 50 iterações.
2. **Uso de Acumulador e Média (25 pts):** Soma correta dos valores e cálculo da média (atenção à divisão float).
3. **Uso de Contador (20 pts):** Incremento correto para unidades > 12 minutos.
4. **Uso de Flag (15 pts):** Ativação correta da flag para casos > 20 minutos (deve ser inicializada em 0 e mudar para 1).
5. **Entrada e Saída (15 pts):** Uso de scanf dentro do loop e printf formatado para os indicadores.
...
setInterval(() => {
  if (editor) {
    localStorage.setItem('codigo_desafio_7', editor.getValue());
  }
}, 2000);

require(['vs/editor/editor.main'], function () {
  const salvo = localStorage.getItem('codigo_desafio_7');
